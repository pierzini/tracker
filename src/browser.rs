...
impl BrowserHistControl {
    ...
    ) -> Result<BrowserHistControl, Box<dyn std::error::Error>> {
    ...
    pub fn update(&mut self) -> Result<usize, Box<dyn std::error::Error>> {
        ...
        let database = file_copy(&self.database)?;
        let con = sqlite::open(&database)?;
        ...
        loop {
            match stmt.next().unwrap() {
                sqlite::State::Row => {
                    ...
                    self.history.push(BrowserHistEntry {
                        timestamp,
                        url,
                        visit_count,
                    });
        ...
        if n != 0 {
            let l_timestamp = self.history.last().unwrap().timestamp;
            self.l_timestamp = l_timestamp;
        }
